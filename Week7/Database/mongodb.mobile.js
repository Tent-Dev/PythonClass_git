...
db.Products.find({},{'pname':1, 'pprice':1}); //แสดงบาง column
db.Products.find({},{'pname':1, 'pprice':1}).limit(5); //แสดงบาง column
db.Products.find({}).sort({'pprice':-1,'pbrand':1}).limit(5);
db.Products.find({'pprice':2500}); // Price = 2500
db.Products.find({'pprice':{'$gt':10000}}); //มากกว่า 10000


db.Products.find({'psize':{'$lt':6}}); //ขนาดหน้าจอน้อยกว่า6 นิ้ว
db.Products.find({'pbrand':{'$in':['Apple','Huawei','LG']}});// brand -> apple, Huawei, LG เรียงราคาจากมากไปน้อย
db.Products.find({'psize.x':3.5});

db.Products.insert({'pid':'p042','year':2562});
db.Products.find({'year':{'$exists':true}}); //เช็คว่ามีข้อมูลนี้อยู่

db.Products.find({'$and':[{'pprice':{'$gt':10000}}, {'psizce':{'$gt':6}}]}) //ราคามากกว่า 10k และ size มากกว่า 6นิ้ว

db.Products.find({'$or':[{'pbrand':'LG'},{'pprice':{'$lt':6000}}]})//แสดงข้อมูล LG หรือ brand อื่นที่มีราคาน้อยกว่า 6000